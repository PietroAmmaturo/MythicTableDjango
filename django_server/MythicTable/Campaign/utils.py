...
class CampaignUtils:
    ...
    def create_default_campaign(
        ...
        created_collection = collection_provider.create_by_campaign(
            ...
            item=MapUtils.create_map(
                "/static/assets/tutorial/thank-you.jpg", 37, 25, 140)
        )
    ...
    def generate_join_id(campaign_provider: MongoDbCampaignProvider):
        ...
        while numAttempts < 5:
            try:
                ...
                return join_id
        raise Exception(
            "Could not find a Join Id in a reasonable time. Try again later.")
    ...
    def create_tutorial_campaign(campaign_provider: MongoDbCampaignProvider, collection_provider: MongoDbCollectionProvider, owner):
        ...
        for character in characters:
            c = collection_provider.create_by_campaign(
                ...
                "characters",
                str(campaign._id),
                CharacterUtil.create_collection_character(
        ...
        map1 = MapUtils.create_map(
            "/static/assets/tutorial/basic-map-interactions.jpg", 37, 25, 140)
        map1["start"] = {"n": 2, "w": 2, "s": 12, "e": 18}
        map1 = collection_provider.create_by_campaign(
            str(owner), "maps", str(campaign._id), map1)

        # Create map2 using MapUtils.create_map()
        map2 = MapUtils.create_map(
            "/static/assets/tutorial/drawing-tools-chat.jpg", 37, 25, 140)
        map2["start"] = {"n": 2, "w": 2, "s": 12, "e": 18}
        map2 = collection_provider.create_by_campaign(
            str(owner), "maps", str(campaign._id), map2)

        # Create map3 using MapUtils.create_map()
        map3 = MapUtils.create_map(
            "/static/assets/tutorial/thank-you.jpg", 37, 25, 140)
        map3 = collection_provider.create_by_campaign(
            str(owner), "maps", str(campaign._id), map3)

        # Create tokens
        tokens = [
            ("marc", "Marc", "Human fighter", "circle", "#000000", str(
                created_characters[0]["_id"]), map1["_id"], 5, 7, None),
            ("sarah", "Sarah", "Elven wizard", "circle", "#1ba73e", str(
                created_characters[1]["_id"]), map1["_id"], 28, 19, None),
            ("Wolf", "Wolf", "Big bad wolf", "circle", "#c02d0c", str(
                created_characters[3]["_id"]), map1["_id"], 25, 6, None, 3),
            ("Redcap", "Goblin 1", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map1["_id"], 27, 7, None, 1),
            ("Redcap", "Goblin 2", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map1["_id"], 26, 8, None, 1),
            ("Redcap", "Goblin 3", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map1["_id"], 24, 6, None, 1),
            ("sarah", "Sarah", "Elven wizard", "circle", "#1ba73e", str(
                created_characters[1]["_id"]), map2["_id"], 6, 6, None),
            ("Redcap", "Goblin 1", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map2["_id"], 11, 6, None, 1),
            ("Redcap", "Goblin 2", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map2["_id"], 11, 7, None, 1),
            ("Redcap", "Goblin 3", "Goblin rogue", "circle", "#c02d0c", str(
                created_characters[2]["_id"]), map2["_id"], 10, 6, None, 1),
            ("marcOld", "Marc", "Elven fighter", "circle", "#3802b8", str(
                created_characters[5]["_id"]), map2["_id"], 26, 7, None),
            ("Tauren", "Ogre", "Ogre mauler", "circle", "#1ba73e", str(
                created_characters[4]["_id"]), map2["_id"], 28, 6, None, 3)
        ]
        for token in tokens:
            collection_provider.create_by_campaign(
                ...
                "tokens",
                str(campaign._id),
                CharacterUtil.create_collection_token(
                    token[0], token[1], token[2], token[3], token[4], token[5], token[6], token[7], token[8], token[9])
            )

        collection_provider.create_by_campaign(str(owner), "players", str(
            campaign._id), MapUtils.create_player(map1["_id"], owner))
