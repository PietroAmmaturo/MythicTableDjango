...
class JsonPatchTranslator:
    def json_path_to_mongo_path(self, path: str) -> str:
        new_path = path[1:].replace("/", ".")
        new_path = re.sub("\\.$", "", new_path, count=1)
        return new_path

    def json_path_to_mongo_character_path(self, path: str) -> str:
        new_path = path[1:]
    ...
    def json_path_to_mongo_array_name(self, path: str) -> str:
        formatted_path = self.json_path_to_mongo_path(path)
        return formatted_path[: formatted_path.rfind(".")]

    def path_is_array(self, path: str) -> bool:
        path_arr = path.split("/")
        return path_arr[-1].isdigit()

    def json_to_bson(self, json_obj):
        if json_obj is None:
        ...
        if isinstance(json_obj, list):
            return [self.json_to_bson(item) for item in json_obj]
